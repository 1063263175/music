...
class Article extends Permissions
{
    ...
    public function comment()
    {
        ...
        $message =  $model->order('add_time desc')->paginate(20) ;
        ...
        if($id > 0) {
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post,['id'=>$id])) {
                    ...
                    //addlog($model->id);//写入日志
                    return $this->success('提交成功','admin/article/comment');
                }
            }
        }
    }
    ...
    public function delComment()
    {
        if($this->request->isAjax()) {
            $id = $this->request->has('id') ? $this->request->param('id', 0, 'intval') : 0;
            if(false == Db::name('article_comment')->where('id',$id)->delete()) {
                return $this->error('删除失败');
            } else {
                //addlog($id);//写入日志
                return $this->success('删除成功','admin/article/comment');
            }
        }
    }
}
