...
use app\admin\model\Comment as Messages;
class Tomessages extends Permissions
{
    public function index()
    {
        ...
        $message = empty($where) ? $model->order('add_time desc')->paginate(20) : $model->where($where)->order('create_time desc')->paginate(20,false,['query'=>$this->request->param()]);
    ...
    public function delete()
    {
    	if($this->request->isAjax()) {
    		$id = $this->request->has('id') ? $this->request->param('id', 0, 'intval') : 0;
            if(false == Db::name('comment')->where('id',$id)->delete()) {
